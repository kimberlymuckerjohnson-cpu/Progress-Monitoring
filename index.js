...
const fluencyAssessments = []; // { id, studentId, date, totalWordsAttempted, errors, wcpm, accuracyPercent }

// very simple auth (not secure for real production)
...
app.post("/students/:id/delete", requireLogin, (req, res) => {
  ...
  if (index !== -1) {
    ...
    // remove related goals and assessments
    for (let i = goals.length - 1; i >= 0; i--) {
      if (goals[i].studentId === studentId) goals.splice(i, 1);
    }
...
/**
 * GENERAL ASSESSMENT – GENERATE ITEMS
 */
...
app.post("/assessments/general/generate", requireLogin, (req, res) => {
  ...
  const generatedItems = selectedGoals.map((g) => ({
    goalId: g.id,
    goalArea: g.area,
...
app.post("/assessments/general/save", requireLogin, (req, res) => {
  ...
  const goalIds = Array.isArray(req.body.goalId)
    ? req.body.goalId
    : [req.body.goalId];
  const prompts = Array.isArray(req.body.prompt)
    ? req.body.prompt
    : [req.body.prompt];
  const correctAnswers = Array.isArray(req.body.correctAnswer)
    ? req.body.correctAnswer
    : [req.body.correctAnswer];
  const scores = Array.isArray(req.body.score)
    ? req.body.score
    : [req.body.score];

  const items = goalIds.map((goalId, i) => ({
...
app.get("/reports", requireLogin, (req, res) => {
  const { tab } = req.query;
  if (tab === "class") return res.redirect("/reports/class");
  return res.redirect("/reports/student");
});
